...
def job_config = [
        job: [
                name: "asdsa2312dsad_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdsa2312dsad_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdsa2312dsad_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdsa2312dsad.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdsa2312dsad", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdsa2312dsad_Pipeline_develop");

job.with {

    authenticationToken('ykIZhiAOJmpx8PnzlwG9BxSnYepCRizB')
